...
rule all:
    input:
        expand(os.path.join(project_dir, "{sample}", "{sample}_" + config["output_suffix"] + "_metrics_with_ols.txt"), sample=name_list)
...
rule Benchmark:
    ...
    output:
        output_benchmark=temp(os.path.join(project_dir, "{sample}", "{sample}_" + config["output_suffix"] + "_benchmark_with_ols.h5ad")),
        output_metrics=os.path.join(project_dir, "{sample}", "{sample}_" + config["output_suffix"] + "_metrics_with_ols.txt"),
    shell: """
        scExtract benchmark \
